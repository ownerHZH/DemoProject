...
public class CommunityPolicitalActivity extends MainAcitivity {
	...
	public void initTabHost() {
        ...
        tabHost.addTab(tabHost
                .newTabSpec("tab2")
                .setIndicator(getString(R.string.community_policital_law),
                		null)
		...
		new Thread() {
			...
			public void run() {
				...
				HttpGet get = new HttpGet(
						AppConstants.HttpHostAdress+"ZganContent.aspx?did=15923258890");
				...
				try {
					...
					if (line.getStatusCode() == 200) {
						...
						if ("0".equals(status)) {
							...
							String conStr=jsonObject2.getString("ContentData");
							if(isNotNull(conStr))
							{
								JSONArray ContentDatacontent = new JSONArray(conStr);
								if(ContentDatacontent.length()>0)
								{
									for(int i=0;i<ContentDatacontent.length();i++)
									{
										ContentData c=new ContentData();
										JSONObject json=ContentDatacontent.getJSONObject(i);
										c.setCContent(json.getString("CContent"));
										c.setCID(json.getString("CID"));
										c.setContentTime(json.getString("ContentTime"));
										c.setPublishers(json.getString("Publishers"));
							...
							if(isNotNull(msStr))
							{
								JSONArray MSZW_BGDDcontent = new JSONArray(msStr);
								if(MSZW_BGDDcontent.length()>0)
								{
									for(int i=0;i<MSZW_BGDDcontent.length();i++)
									{
										MSZW_BGDD c=new MSZW_BGDD();
										JSONObject json=MSZW_BGDDcontent.getJSONObject(i);
										c.setAddLX(json.getString("AddLX"));
										c.setAddress(json.getString("Address"));
										c.setCID(json.getString("CID"));
										c.setGPSCoordinates(json.getString("GPSCoordinates"));
										c.setPublishers(json.getString("Publishers"));
										c.setPublishTime(json.getString("PublishTime"));
